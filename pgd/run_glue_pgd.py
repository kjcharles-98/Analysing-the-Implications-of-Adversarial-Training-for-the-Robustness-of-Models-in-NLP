    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            for astep in range(args.adv_steps):
                ...
                loss, tr_logits = outputs[:2]  # model outputs are always tuple in transformers (see doc)
                #print("tr_logits:",tr_logits.shape) torch.Size([8, 2])  
                #print("labels:",inputs['labels'].shape) torch.Size([8])
                # test
                ...
                if args.norm_type == "l2":
                    ...
                    # modify start
                    # cand_max = max(torch.norm(delta.view(delta.size(0), -1).float(), p=2, dim=1).detach()).item()
                    # print("debug_norm:",cand_max)
                    #if cand_max > global_max_norm:
                    #    global_max_norm = cand_max
                    #    print("New global_max_norm: ",cand_max)
                    # modify end
                    if args.adv_max_norm > 0:
                        ...
                        #d_norm = torch.norm(delta.view(delta.size(0), -1).float(), p=2, dim=1).detach()
                        #print("d_norm",d_norm)
                elif args.norm_type == "linf":
                    denorm = torch.norm(delta_grad.view(delta_grad.size(0), -1), dim=1, p=float("inf")).view(-1, 1, 1)
                    denorm = torch.clamp(denorm, min=1e-8)
                    delta = (delta + args.adv_lr * delta_grad / denorm).detach()
                    #print("Norm type: Linf")
                    #print("delta:",delta)
                    if args.adv_max_norm > 0:
